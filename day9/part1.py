...
for r in range(numRows):
    for c in range(numCols):
        ...
        if all([data[(r, c)] < data[(r + deltar, c + deltac)] for deltar in [-1, 0, 1] for deltac in [-1, 0, 1] if (deltar*deltac == 0 and deltar != deltac)]):
            print(f"Low Point Found: ({r},{c}) : {data[(r, c)]}")
            total += data[(r, c)] + 1
        else:
            print(f"Not: ({r},{c}) : {data[(r, c)]}", end = " ")
            print(f" {[data[(r + deltar, c + deltac)] for deltar in [-1, 0, 1] for deltac in [-1, 0, 1] if (deltar*deltac == 0 and deltar != deltac)]}")
